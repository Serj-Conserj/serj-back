...
async def process_relationships(session, place, place_data):
    ...
    for name in place_data["alternate_name"]:
        alt_name = await session.run_sync(
            lambda s: s.query(AlternateName).filter_by(name=name).first()
        )
        if not alt_name:
    ...
    for metro in place_data["close_metro"]:
        metro_obj = await session.run_sync(
            lambda s: s.query(MetroStation).filter_by(name=metro).first()
        )
        if not metro_obj:
    ...
    for cuisine in place_data["main_cuisine"]:
        cuisine_obj = await session.run_sync(
            lambda s: s.query(Cuisine).filter_by(name=cuisine).first()
        )
        if not cuisine_obj:
    ...
    for feature in place_data["features"]:
        feature_obj = await session.run_sync(
            lambda s: s.query(Feature).filter_by(name=feature).first()
        )
        if not feature_obj:
    ...
    for purpose in place_data["visit_purposes"]:
        purpose_obj = await session.run_sync(
            lambda s: s.query(VisitPurpose).filter_by(name=purpose).first()
        )
        if not purpose_obj:
